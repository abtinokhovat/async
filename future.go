...
type Future[T any] struct {
	resolved int32 // 0 = not resolved, 1 = resolved
	res      *result[T]
...
func (f *Future[T]) Resolve(val T, err error) {
	if atomic.CompareAndSwapInt32(&f.resolved, 0, 1) {
...
func (f *Future[T]) Get(ctx context.Context) T {
	...
	if atomic.LoadInt32(&f.resolved) == 1 && f.res != nil && f.res.val != nil {
		return *f.res.val
	}
...
func (f *Future[T]) Error(ctx context.Context) error {
	if atomic.LoadInt32(&f.resolved) == 1 && f.res != nil {
		return f.res.err
	}
...
func (f *Future[T]) IsResolved() bool {
	return atomic.LoadInt32(&f.resolved) == 1
}
